...
if ! command -v mysql &> /dev/null; then
    echo "错误: 未找到MySQL命令行工具，请确保MySQL已正确安装"
    exit 1
fi
...
echo
echo "1. 创建数据库表结构..."
if ! mysql -h"$DB_HOST" -P"$DB_PORT" -u"$DB_USER" -p"$DB_PASSWORD" "$DB_NAME" < "$SCRIPT_DIR/create-charity-tables.sql"; then
    echo "错误: 表结构创建失败"
    exit 1
fi
echo "表结构创建成功！"
...
echo
echo "2. 插入测试数据..."
if ! mysql -h"$DB_HOST" -P"$DB_PORT" -u"$DB_USER" -p"$DB_PASSWORD" "$DB_NAME" < "$SCRIPT_DIR/insert-charity-data.sql"; then
    echo "错误: 数据插入失败"
    exit 1
fi
echo "测试数据插入成功！"
